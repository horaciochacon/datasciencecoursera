...
cr <- corr("R_Programming/Week2/data/specdata/")                
cr <- sort(pull(cr,cor))              
set.seed(868)                
out <- round(cr[sample(length(cr), 5)], 4)
print(out)

cr <- corr("R_Programming/Week2/data/specdata/", 129)                
cr <- sort(pull(cr,cor))                  
n <- length(cr)                
set.seed(197)                
...
cr <- corr("R_Programming/Week2/data/specdata/", 2000)                
n <- length(pull(cr,cor))                
cr <- corr("R_Programming/Week2/data/specdata/", 1000)                
cr <- sort(pull(cr,cor))
